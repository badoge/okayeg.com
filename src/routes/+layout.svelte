<script>
  ...
  onMount(() => {
    themeChange(false);
    darkTheme = localStorage.getItem("theme") === "forest";
  });
...
<div class="navbar bg-neutral text-neutral-content shadow-md flex w-full py-0">
  <div class="navbar-start w-45 flex-none">
    <div class="dropdown">
      ...
      <ul class="menu dropdown-content bg-neutral text-neutral-content rounded-box z-1 mt-4 w-65 shadow-sm">
        <NavbarLinks />
      </ul>
    </div>
  ...
  <div class="navbar-end flex-auto">
    <div class="tooltip tooltip-left" data-tip={darkTheme ? "Switch to light theme" : "Switch to dark theme"}>
      <label class="swap swap-rotate m-1">
        ...
        <input data-toggle-theme="lemonade,forest" type="checkbox" class="theme-controller" name="theme" bind:checked={darkTheme} />
