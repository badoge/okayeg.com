<script>
   ...
   * @property {any} cell
   * @property {boolean} [allowFlicks]
   * @property {boolean} [focused]
   * @property {boolean} [isPuzzlePiece] - puzzle piece:
   */
...
<div
  ...
  tabindex="-1"
  onmousedown={(event) => startDrag(cell, event)}
  onmouseup={(event) => endDrag(cell, event)}
  ontouchstart={(event) => startFlick(cell, event)}
  ontouchend={(event) => endFlick(cell, event)}
  onclick={() => doClickCell(cell)}
  onkeypress={(event) => {
