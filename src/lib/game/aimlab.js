...
class Aimlab extends BaseGame {
  ...
  getShareableData() {
    ...
    if (this.stats.runs > 0) {
      str += `\n🏁 Last run: ${formatTimer(this.stats.lastTime, true)} (~${Math.round(this.stats.lastClickTime)} ms)`;
      if (Math.round(this.stats.lastClickTime) === Math.round(this.stats.bestClickTime)) str += "🎖️";
